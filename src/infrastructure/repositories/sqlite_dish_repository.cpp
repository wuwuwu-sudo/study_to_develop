...
// 从结果集当前行读取一个 Dish。
// 列顺序固定为：id, merchant_id, name, price, category, description, available, deleted
...
std::optional<Dish> SqliteDishRepository::find_by_id(int dish_id) {
    ...
    const char* sql = "SELECT id, merchant_id, name, price, category, description, available, deleted FROM dishes WHERE id = ?;";
...
std::vector<Dish> SqliteDishRepository::find_by_merchant(int merchant_id) {
    ...
    sqlite3* handle = conn->handle();
    const char* sql = "SELECT id, merchant_id, name, price, category, description, available, deleted "
...
bool SqliteDishRepository::set_available(int dish_id, bool available) {
    ...
    sqlite3* handle = conn->handle();
    const char* sql = "UPDATE dishes SET available = ? WHERE id = ? AND deleted = 0;";
...
std::vector<Dish> SqliteDishRepository::find_paged(int merchant_id, int page, int page_size) {
    ...
    sqlite3* handle = conn->handle();
    const char* sql =
        "SELECT id, merchant_id, name, price, category, description, available, deleted "
        "FROM dishes WHERE merchant_id = ? AND deleted = 0 "
